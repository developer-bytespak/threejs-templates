...
import { resolveShots } from './shots.js'

const WORLD_UP = new THREE.Vector3(0, 1, 0)
const FALLBACK_UP = new THREE.Vector3(0, 0, 1)

// How hard the camera resists the scroll. Lower is heavier and more filmic;
// this is the difference between a dolly and a cut.
const POSITION_DAMPING = 2.6
const LOOK_DAMPING = 3.2
const PARALLAX_DAMPING = 2.2

// Metres the cursor can push the camera off its rail, and the idle sway that
// keeps the frame alive when nobody is touching anything.
const PARALLAX_REACH = 0.16
const DRIFT_REACH = 0.022
...
function CinematicCamera({ input, reducedMotion }) {
  const { focus, bounds } = useRoomModel()
  const shots = useMemo(() => resolveShots(focus, bounds), [focus, bounds])
  ...
  useFrame((state, delta) => {
    ...
    // Locate the current shot pair and ease within it, so the camera settles
    // at each keyframe rather than sliding through at constant speed.
    const span = shots.length - 1
    const travelled = THREE.MathUtils.clamp(input.current.progress, 0, 1) * span
    const index = Math.min(Math.floor(travelled), span - 1)
    const blend = easeInOutCubic(travelled - index)
    ...
    if (!reducedMotion) {
      ...
      scratch.position.y += Math.sin(time * 0.34) * DRIFT_REACH
      scratch.position.addScaledVector(
        scratch.right,
        Math.sin(time * 0.21) * DRIFT_REACH,
      )
    }
